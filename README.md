...
[![asciicast](https://asciinema.org/a/cNbwyorYuzFRXhpwx72vBuvOe.svg)](https://asciinema.org/a/cNbwyorYuzFRXhpwx72vBuvOe)
...
```sh
make docker-build docker-push IMG=<some-registry>/custom-secret-controller:tag
```
...
```sh
make deploy IMG=<some-registry>/custom-secret-controller:tag
```
...
```sh
make build-installer IMG=<some-registry>/custom-secret-controller:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/custom-secret-controller/<tag or branch>/dist/install.yaml
```
